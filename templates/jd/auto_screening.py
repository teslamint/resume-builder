...
8) 저장용 분석 문서처럼 작성하고, 사용자에게 말을 거는 문장이나 후속 제안 문장을 쓰지 않음
9) `원하시면`, `해드리겠습니다`, `다음 단계로`, `~할 수 있습니다` 같은 대화형 문구 금지
10) 판정은 단호하게 유지하되 회사나 포지션을 평가절하하지 말고, 후보자의 기준과 맞는지 중심으로 설명
11) `리드 전가 리스크`, `금융 리스크`, `즉시 컷` 같은 내부 기준 용어는 필요한 경우에만 쓰고, 근거 문장은 자연스러운 한국어로 풀어 씀
12) `## 핵심 근거`는 3~5개의 짧은 문단 또는 불릿으로 압축하고, 감사 로그처럼 딱딱한 표현을 피함
