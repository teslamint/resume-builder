...
uv run python -m templates.jd.auto --from-urls <url-file>

# 중복/수동 필요 여부만 확인
uv run python -m templates.jd.pipeline --file <url-file>
```
...
uv run python -m templates.jd.pipeline --rescreen private/job_postings/pass/

# 미리보기 (dry-run)
uv run python -m templates.jd.pipeline --rescreen private/job_postings/pass/ --dry-run
```

### 3. 현재 상태 확인

```bash
uv run python -m templates.jd.pipeline --status
```
...
```bash
uv run python -m templates.jd.pipeline --url "{URL}"
```
...
```bash
uv run python -m templates.jd.wanted_extract <job_id>
```
...
uv run python -m templates.jd.remember_batch_extract /tmp/remember_url.txt
```
...
3. 없으면 `/extract-company-info` 스킬 워크플로우로 풀 추출:
   ...
   **d) 검증 (필수)**:
   ```bash
   uv run python -m templates.jd.company_validator --file private/company_info/{company_slug}.md --fix
   ```
...
```bash
uv run python -m templates.jd.pipeline --classify private/job_postings/unprocessed/
```
