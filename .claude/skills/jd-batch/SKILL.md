...
- 이 스킬은 resume repo 루트(`/Users/teslamint/workspace/resume`)에서 사용한다.
- Python 명령은 기본적으로 `uv run python`을 사용한다. 시스템 `python3`는 구버전일 수 있어 타입 힌트 문법에서 실패할 수 있다.
- URL 입력은 가능하면 통합 자동 파이프라인을 먼저 사용한다.
- 웹 자동화 fallback이 필요하면 repo 지침의 `agent-browser`를 사용한다.
...
```bash
# URL 목록 파일은 사용자가 제공한 파일을 쓰거나, repo 내 unprocessed/에 생성한다.
# 예: private/job_postings/unprocessed/batch_YYYYMMDD_HHMM.txt

# 통합 처리 실행 (추출 → 회사정보 → 스크리닝 → 자동 분류)
uv run python templates/jd/auto.py --from-urls <url-file>

# 중복/수동 필요 여부만 확인
uv run python templates/jd/pipeline.py --file <url-file>
```
...
uv run python templates/jd/pipeline.py --rescreen private/job_postings/pass/

# 미리보기 (dry-run)
uv run python templates/jd/pipeline.py --rescreen private/job_postings/pass/ --dry-run
```

### 3. 현재 상태 확인

```bash
uv run python templates/jd/pipeline.py --status
```
...
| Phase | 작업 | 도구 |
|-------|------|------|
| 1 | URL 처리/중복 체크 | `auto.py --from-urls` 또는 `pipeline.py --url` |
| 2 | JD 추출 + 회사 URL 추출 | HTTP extractor 우선, 필요 시 agent-browser fallback |
| 3 | 회사 정보 | `private/company_info/` 확인 → 없으면 자동 파이프라인 또는 agent-browser 검색 |
| 4 | 스크리닝 | jd-screening-rules.md 로드 → LLM 분석 |
| 5 | 자동 분류 | `pipeline.py --classify` |

### Phase 1: 중복 체크

```bash
uv run python templates/jd/pipeline.py --url "{URL}"
```
...
> **전환 정책 (2026-04-17)**: HTTP 우선 + browser fallback 구조. Wanted/Remember는 이미 production에서 `auto.py`가 사용 중인 HTTP 모듈을 직접 호출하여 15~30배 빠름. R1 감사 실측 33건 ~15초(건당 0.5초).
...
```bash
uv run python templates/jd/wanted_extract.py <job_id>
```
...
uv run python templates/jd/remember_batch_extract.py /tmp/remember_url.txt
```
...
#### 방법 B — agent-browser fallback

방법 A 실패(`status != ok`, HTTP 404, 파싱 오류) 또는 **Saramin / Jumpit / JobKorea** 등 미지원 플랫폼일 때:

1. `agent-browser open <url>`
2. `agent-browser snapshot -i`
3. 페이지 텍스트와 링크를 확인해 채용공고 정보 파싱
4. 회사 프로필 URL 또는 회사명 추출
5. 필요 시 `agent-browser`로 회사 정보 검색
...
- `agent-browser snapshot -i` 결과에서 위 selector에 해당하는 링크를 확인한다.
...
3. 없으면 `/extract-company-info` 스킬 워크플로우로 풀 추출:
   ...
   **d) 검증 (필수)**:
   ```bash
   uv run python templates/jd/company_validator.py --file private/company_info/{company_slug}.md --fix
   ```
...
```bash
uv run python templates/jd/pipeline.py --classify private/job_postings/unprocessed/
```
...
| 상황 | 처리 |
|------|------|
| 방법 A (HTTP) 추출 실패 (status != ok) | 방법 B agent-browser fallback 진입 |
...
| agent-browser 연결 실패 | 브라우저 상태 확인 후 대기 |
