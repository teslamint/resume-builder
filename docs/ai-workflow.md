...
| `/extract-recruitment-info` | 채용 정보 통합 추출 (회사 + JD) | `private/company_info/` + `private/job_postings/` |
...
`templates/jd/auto.py` 기본 실행은 검색 단계와 URL 파일 기반 후속 처리 단계를 분리해서 수행합니다.
...
# 기존 JD만 스크리닝/분류
python -m templates.jd.auto --screening-only --from-urls private/job_postings/unprocessed/search_YYYYMMDD_HHMM.txt

# 이전 실행에서 미완료 항목만 재처리
python -m templates.jd.auto --resume

# 미리보기 모드 (파일 저장/이동 최소화)
python -m templates.jd.auto --dry-run

# 검색만 실행
python -m templates.jd.auto --search-only
```
...
python -m templates.jd.auto --thevc-mode auto

# TheVC 투자정보 수집 건너뛰기
python -m templates.jd.auto --thevc-mode skip

# TheVC 투자정보가 반드시 필요할 때
python -m templates.jd.auto --thevc-mode require

# 로그인 후 보완 큐 재처리
python -m templates.jd.auto --company-enrichment-only --thevc-mode require
```
...
python -m templates.jd.auto --min-completeness 60

# 검색 스크립트
python -m templates.jd.search --query "백엔드 시니어" --dry-run
python -m templates.jd.search --status
```
...
| 모듈 | 역할 |
|------|------|
| `auto.py` | 전체 파이프라인 오케스트레이션 |
| `search.py` | Wanted/Remember 검색 |
| `search_helpers.py` | 검색 공통 유틸 (페이지 로드, DOM 스크래핑) |
| `jd_content.py` | JD 콘텐츠 파싱, 메타데이터, 프론트매터 |
| `experience_filter.py` | 경력 범위 필터링 (`search_config.yaml` 기반) |
| `notifications.py` | 파이프라인 완료 알림 |
| `models.py` | 도메인 모델 (`DiscoveredJob`, `JobPosting`, `QueueItem`) |
| `pipeline.py` | 스크리닝 결과 기반 파일 분류 |
